	...
	const FMcpToolContext& Context)
{
	...
	UE_LOG(LogYesUeMcp, Log, TEXT("set-property: %s.%s (component=%s, clear=%s)"),
	...
	if (Blueprint)
	{
		...
		if (!ComponentName.IsEmpty())
		{
						...
						if (AActor* ParentCDO = Cast<AActor>(ParentClass->GetDefaultObject()))
						{
							...
							for (UActorComponent* Comp : ParentComponents)
							{
								if (Comp && Comp->GetName() == ComponentName)
								{
									...
									UE_LOG(LogYesUeMcp, Log, TEXT("set-property: Component '%s' found in parent, creating override"), *ComponentName);
			...
			if (!bFoundComponent)
			{
				return FMcpToolResult::Error(FString::Printf(TEXT("Component '%s' not found in Blueprint"), *ComponentName));
			}
		}
		else
		{
	...
	if (bClearOverride)
	{
		...
		if (DefaultComponent)
		{
			...
			if (DefaultValuePtr && TargetValuePtr)
			{
				...
				UE_LOG(LogYesUeMcp, Log, TEXT("set-property: Cleared override for %s.%s"), *AssetPath, *PropertyPath);
			}
			else
			{
	...
	UE_LOG(LogYesUeMcp, Log, TEXT("set-property: Successfully %s %s.%s%s"),

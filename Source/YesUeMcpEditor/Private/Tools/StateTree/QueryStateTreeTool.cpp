	...
	const FMcpToolContext& Context)
{
	...
	UE_LOG(LogYesUeMcp, Log, TEXT("query-statetree: path='%s', include='%s'"), *AssetPath, *Include);

	// Load the StateTree asset
	UStateTree* StateTree = LoadObject<UStateTree>(nullptr, *AssetPath);
	if (!StateTree)
	{
		UE_LOG(LogYesUeMcp, Warning, TEXT("query-statetree: Failed to load StateTree at '%s'"), *AssetPath);

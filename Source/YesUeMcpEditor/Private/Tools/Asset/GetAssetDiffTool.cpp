	...
	const FMcpToolContext& Context)
{
	...
	UE_LOG(LogYesUeMcp, Log, TEXT("get-asset-diff: path='%s', scm='%s', revision='%s'"),
...
UGetAssetDiffTool::EScmType UGetAssetDiffTool::DetectScmType() const
{
	...
	{
		UE_LOG(LogYesUeMcp, Log, TEXT("Detected Git repository"));
		return EScmType::Git;
	}
	...
	{
		UE_LOG(LogYesUeMcp, Log, TEXT("Detected Perforce workspace"));
...
bool UGetAssetDiffTool::ExtractGitBaseVersion(const FString& FilePath, const FString& Revision, FString& OutTempPath, FString& OutError) const
{
	...
	UE_LOG(LogYesUeMcp, Log, TEXT("Extracted Git base version to: %s (%lld bytes)"), *OutTempPath, FileSize);
	return true;
}
...
bool UGetAssetDiffTool::ExtractPerforceBaseVersion(const FString& FilePath, const FString& Revision, FString& OutTempPath, FString& OutError) const
{
	...
	UE_LOG(LogYesUeMcp, Log, TEXT("Extracted Perforce base version to: %s (%lld bytes)"), *OutTempPath, FileSize);
	return true;
}
...
void UGetAssetDiffTool::CleanupTempFile(const FString& TempPath) const
{
	if (!TempPath.IsEmpty() && FPaths::FileExists(TempPath))
	{
		...
		UE_LOG(LogYesUeMcp, Verbose, TEXT("Cleaned up temp file: %s"), *TempPath);
	}
}
